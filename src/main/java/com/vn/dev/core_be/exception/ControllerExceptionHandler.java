...
public class ControllerExceptionHandler {
    ...
    public ApiBaseResponse argumentException(Exception ex, WebRequest request) {
    ...
    public ApiBaseResponse existedException(MasterException ex, WebRequest request){
        return new ApiBaseResponse(CODE_EXISTED.getCode(), ex.toString());
    }

    @ExceptionHandler(Exception.class)
    @ResponseStatus(value = HttpStatus.INTERNAL_SERVER_ERROR)
    public String exception(MasterException ex, WebRequest request){
        return ex.toString();
    }
}
